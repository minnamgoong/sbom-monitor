...
2. **Syft 바이너리 업데이트**: Nexus3의 `latest_version.txt`를 참조하여 현재 `bin/syft`의 버전과 다를 경우 최신 바이너리를 다운로드하여 교체합니다.

> **참고**: 별도의 버전 파일을 관리할 필요 없이, 중앙 Nexus3 저장소의 `collect-sbom.sh` 파일만 최신으로 교체하면 모든 서버의 에이전트가 다음 실행 시점에 자동으로 최신 코드를 적용하게 됩니다.
