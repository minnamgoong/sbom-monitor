...
- 대상 서버에 스크립트를 다운로드하여 적절한 디렉토리(예: `~/sbom-monitor` 또는 원하는 전용 계정의 디렉토리)에 위치시킵니다.
...
에이전트는 실행될 때마다(수동/자동 무관) 상단에서 자체적으로 버전 확인 로직을 수행합니다.

1. **스크립트 업데이트**: Nexus3에 등록된 버전을 확인하여, 신규 버전이 존재하면 스크립트 자신(`collect-sbom.sh`)을 최신 버전으로 덮어쓰고 즉시 재기동합니다.
2. **Syft 바이너리 업데이트**: Nexus3에 등록된 최신 오픈소스 `syft` 버전을 확인하여, 구버전인 경우 `bin/syft`를 다운로드하여 교체합니다.

> **참고**: 따라서 인프라 관리자는 대상 서버별로 에이전트를 재배포할 필요 없이, 중앙 Nexus3 저장소의 스크립트 및 Syft 바이너리만 최신으로 갱신하면 향후 에이전트 실행 주기에 맞춰 **자동으로 모든 서버가 최신 버전을 유지**하게 됩니다.
